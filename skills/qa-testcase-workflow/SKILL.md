...
/qa-testcase-workflow --resume               # 从上次中断处继续
```
...
- **选项2：手动创建**
  ...
  创建完成后，重新执行：/qa-testcase-workflow {feature_name}
  ```
...
function validate_prd_file(feature_name):
  ...
  if len(prd_files) == 0:
    ...
    添加PRD后，重新执行：/qa-testcase-workflow {feature_name}
    "
    return false
  ...
  for step in steps_to_execute:
    if step > 1:
      ...
      if not file_exists(required_artifact):
        ...
        解决方案：
          [1] 先执行步骤{step-1}：/qa-testcase-workflow --steps {step-1}
          [2] 从步骤1完整执行：/qa-testcase-workflow {feature_name}
          [3] 忽略依赖检查（慎用）：/qa-testcase-workflow --from {step} --ignore-validation
        "
        return false
...
4. 处理用户选择：
   ...
   选择3 (abort)：
     ...
     - 显示："工作流已终止，状态已保存。"
     - 显示："修复问题后，使用以下命令继续："
     - 显示："  /qa-testcase-workflow --resume"
     - 退出工作流
