...
本文档提供QA测试用例工作流（`/qa-testcase-workflow`）在执行过程中可能遇到的常见错误场景及其解决方案。
...
```bash
# 从步骤2开始执行
/qa-testcase-workflow --from 2 退款需求
```

**方案B：跳过依赖检查（慎用）**
```bash
# 如果你确认产物实际存在或不需要
/qa-testcase-workflow --from 3 --ignore-validation 退款需求
```
